  ...
  <div style="background:#eeeeee;padding-top:10px;min-height:1000px;height:auto;">
    ...
    >
      <div style="padding-bottom:10px;padding-top:10px;float:left;width:100%;">
        <el-row>
          <el-col :span="3">
            <div style="margin-top:3px;margin-left:0;font-size:18px;text-align:center;">用户进出门记录</div>
          </el-col>
          <el-col :span="4">
      ...
      >
        ...
        <el-table-column prop="trace_type" label="进/出" width="120">
          <template slot-scope="scope">
            <span v-if="scope.row.trace_type == 0">进门</span>
            <span v-if="scope.row.trace_type == 1">出门</span>
          </template>
        </el-table-column>
        <el-table-column prop="in_time" label="时间" width="160">
          <template slot-scope="scope">
            <span v-if="scope.row.trace_type == 0">{{scope.row.in_time}}</span>
            <span v-if="scope.row.trace_type == 1">{{scope.row.out_time}}</span>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    getData(data) {
      ...
      console.log(data);
      request({
        url: concans.schema+"://" + concans.host + "/contract/adminOrder/traceList",
        method: "post",
        data: data,
      })
        .then((res) => {
